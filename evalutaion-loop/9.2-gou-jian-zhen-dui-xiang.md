...
| 字段                | 类型                 | 功能                               |
| ----------------- | ------------------ | -------------------------------- |
| f\_back           | PyFrameObject \*   | 指向栈中前一个帧的指针，如果是第一帧则值为null        |
...
| f\_stacktop       | PyObject \*\*      | 指向 f\_valuestack 中下一个空闲的插槽(slot) |
...
| f\_lineno         | int                | 当前行号                             |
| f\_iblock         | int                | 当前帧在 f\_blockstack 中的索引          |
| f\_executing      | char               | 指示当前帧是否仍在执行的标志位                  |
| f\_blockstack     | PyTryBlock\[]      | 保存for，try和循环块的序列                 |
...
* **globals:** 类型为 `PyObjecy*`(实际类型为一个字典)，字典的键为变量名，字典的值是变量的值；
* **locals:** 类型为 `PyObjecy*`(实际类型为一个字典)，字典的键为变量名，字典的值是变量的值。
